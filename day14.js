...
*/
function getNextRecipes(after, nextCount=10) {
  var recipes = [3, 7];
  var recipeCount = 2;
  var e1 = 0;
  var e2 = 1;
  recipes.length = nextCount + after;
  while (recipeCount < recipes.length) {
    var newRecipes = recipes[e1] + recipes[e2];
    if (newRecipes < 10) {
      recipes[recipeCount++] = newRecipes;
    } else {
      recipes[recipeCount++] = 1;
      recipes[recipeCount++] = newRecipes - 10;
    }
...
console.assert(getNextRecipes(5) == "0124515891");
