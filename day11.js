...
function computeGridPower(serialNumber) {
  var grid = [[]];
  for (var y = 1; y<=300; y++) {
    var row = [];
    for (var x = 1; x<=300; x++) {
      row[x] = getPower(x, y, serialNumber);
    }
    grid[y] = row;
  }
  return grid;
...
function getMaxSquare(grid) {
  var maxx, maxy, maxPower = -Infinity;
  for (var y = 1; y<grid.length-3; y++) {
    for (var x = 1; x<grid[y].length-3; x++) {
      var squarePower = computeSquarePower(grid, x, y);
...
console.assert(getMaxSquare([[],
  [0,-2,  -4,   4,   4,   4],        
  [0,-4,   4,   4,   4,  -5],
  [0, 4,   3,   3,   4,  -4],
  [0, 1,   1,   2,   4,  -3],
  [0,-1,   0,   2,  -5,  -2]]).toString() == [2,2].toString());  
function getPower(x, y, serialNumber) {
  var rackId = x + 10;
  var power = rackId * y;
  power += serialNumber;
  power *= rackId;
...
console.assert(getPower(3, 5, 8) == 4);
console.assert(getPower(122,79,57) == -5);
console.assert(getPower(217,196,39) == 0);
console.assert(getPower(101,153,71) == 4);

console.assert(getMaxSquare(computeGridPower(18)).toString() == [33,45].toString());
