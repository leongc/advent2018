...
function makeErosionGrid(depth, targetx, targety) {
  let result = [];
  for (let y = 0; y <= targety; y++) {
    let row = [];
    for (let x = 0; x <= targetx; x++) {
      let gi;
      if ((x == 0 && y == 0) || (x == targetx && y == targety)) { gi = 0; }
      else if (y == 0) { gi = x * 16807; }
      else if (x == 0) { gi = y * 48271; }
      else { gi = row[x-1] * result[y-1][x]; }
      let ei = (gi + depth) % 20183;
      row[x] = ei;
    }
    result.push(row);
  }
...
function dumpType(erosionGrid, size = 16) {
  console.log( 
    erosionGrid.slice(0, size+1)
      .map(row => 
        row.slice(0, size+1)
          .map(function(ei) {
            switch (ei % 3) { 
...
function sumRisk(erosionGrid) {
  return erosionGrid.reduce((acc, curRow) => 
    acc + curRow.reduce((a, c) => 
      a + (c % 3), 0), 
    0);
}

console.assert(sumRisk(makeErosionGrid(510, 10, 10)) == 114);
console.log(sumRisk(makeErosionGrid(7863, 14, 760)));
