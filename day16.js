...
*/
console.assert(mulr(2, 1, 2, [3, 2, 1, 1]).toString() == [3, 2, 2, 1].toString());
/*
Opcode 9 could be addi: register 2 (which has a value of 1) plus value 1 produces 2, which matches the value stored in the output register, register 2.
*/
console.assert(addi(2, 1, 2, [3, 2, 1, 1]).toString() == [3, 2, 2, 1].toString());
/*
Opcode 9 could be seti: value 2 matches the value stored in the output register, register 2; the number given for B is irrelevant.
None of the other opcodes produce the result captured in the sample. Because of this, the sample above behaves like three opcodes.
*/
console.assert(seti(2, 1, 2, [3, 2, 1, 1]).toString() == [3, 2, 2, 1].toString());
/*
...
function threeOps(a, b, c, before, after) {
  var possibleOps = 0;
  for (let op of ops) {
    var actual = op.call(undefined, a, b, c, Array.from(before));
    if (actual.toString() == after.toString()) {
      possibleOps++;
      if (possibleOps == 3) { return true; }
    }
  }
  return false;
}
console.assert(threeOps(2, 1, 2, [3, 2, 1, 1], [3, 2, 2, 1]));

function countThreeOpsSamples(lines) {
  var threeOpsSamples = 0;
  var before, opcode, a, b, c, after;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith("Before: ")) {
      before = JSON.parse(lines[i].substring(8));
    } else if (lines[i].startsWith("After: ")) {
      after = JSON.parse(lines[i].substring(7));
      if (threeOps(a, b, c, before, after)) {
        threeOpsSamples++;
      }
    } else if (lines[i] == "") {
      // do nothing
...
console.assert(countThreeOpsSamples(testInput) == 1); 
